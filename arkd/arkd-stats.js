...
function calculateDaysSince() {
    ...
    const referenceDate = new Date('2022-09-15');
    const daysSince = Math.floor((currentDate - referenceDate) / (1000 * 60 * 60 * 24));
    return daysSince;
}

const query1H = `{
    price(token: "arkade.token", period: 1) {
        id
        timestamp
        priceInKda
        intervalStamp
...
const queryDaysSince = `{
    price(token: "arkade.token", period: ${calculateDaysSince()}) {
        id
        timestamp
        priceInKda
        intervalStamp
...
async function fetchData() {
    try {
        ...
        const FDMC = currentPrice * 1000000000;
